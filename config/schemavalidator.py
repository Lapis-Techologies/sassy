...
class SchemaValidator:
    ...
    @staticmethod
    def validate_dict_with_types(value: Any, key_type_pairs: Dict[str, Callable[[Any], bool]], path: str = "", optional_keys: List[str] | None = None) -> List[str]:
        """
        ...
        if not SchemaValidator.is_dict(value):
            errors.append(f"Expected dictionary at '{path}', got {type(value).__name__}")
            return errors
            ...
            if key in key_type_pairs:
                ...
                if callable(validator):
                    if not validator(val):
                        ...
                        errors.append(f"Expected {type_name} for '{current_path}', got {type(val).__name__}")
