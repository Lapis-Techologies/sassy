...
class BotConfig(ConfigHandler):
    ...
    def _verify_config(self, config: dict) -> None:
        ...
        for section in ["database", "bot", "guild", "xp"]:
            if section not in config:
                raise self.error.ConfigError(
                    f"Missing '{section}' configuration section"
                )
    ...
    def _validate_against_schema(self, config: dict) -> None:
        ...
        if (
            "guild" in config
            and "roles" in config["guild"]
            and "reactions" in config["guild"]["roles"]
        ):
            ...
            for key, value in reactions.items():
                if key != "message":  # Skip the message ID key
                    if not isinstance(value, int):
                        errors.append(
                            f"Expected int for reaction role value at 'guild.roles.reactions.{key}', got {type(value).__name__}"
                        )
        ...
        if "xp" in config and "rewards" in config["xp"]:
            ...
            for level, role in rewards.items():
                if not isinstance(level, str) or not level.isdigit():
                    errors.append(
                        f"Invalid reward level '{level}', must be a digit string"
                    )
                if not isinstance(role, int):
                    errors.append(
                        f"Invalid reward role for level '{level}', must be an int"
                    )
    ...
    def _validate_special_cases(self, config: dict) -> None:
        ...
        if (
            ...
            if "message" not in reactions:
                raise self.error.ConfigError(
                    "Missing required key 'message' in 'guild.roles.reactions'"
                )
