...
class Schedule:
    def __init__(self, bot: "Sassy", database_name: str, callback: callable):
        self.bot = bot
        self.database = self.bot.database[database_name]
        self.callback = callback

    async def watch_all_events(self) -> None:
        future_events = self.database.find({"finished": {"$ne": True}})
    ...
    async def finish_event(self, event: dict | InsertOneResult):
        ...
        event = await self.database.find_one({"_id": event["_id"]})
