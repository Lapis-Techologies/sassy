...
class Warnings(commands.Cog):
    ...
    async def find_case_by_id(self, case_id, interaction , member):
        ...
        if curs is None:
            embed = discord.Embed(title="No Warnings Found", description=f"{member.mention} does not have any warnings on record!", color=0x00FF00)
    ...
    async def find_warnings(self, member, interaction) -> None:
        ...
        embed = discord.Embed(title=f"Warnings for {member} ({member.id})", color=0xFF0000)
        count = 0
    ...
    @app_commands.command(name="warnings", description="View all warnings for a specific user.")
    @db_check()
    @is_admin()
    async def warnings(self, interaction: Interaction, member: discord.Member, case_id: str | None = None) -> None:
